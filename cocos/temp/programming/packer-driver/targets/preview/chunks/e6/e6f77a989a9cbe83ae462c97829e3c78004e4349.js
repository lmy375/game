  ...
  return {
    ...
    execute: function () {
      ...
      _export("EnemyAI", EnemyAI = class EnemyAI {
        ...
        planTurn(state) {
          var actorId = state.activeUnitId;
          var actor = actorId ? (_crd && unitById === void 0 ? (_reportPossibleCrUseOfunitById({
            error: Error()
          ...
          var plan = this.bestPlanForUnit((_crd && cloneState === void 0 ? (_reportPossibleCrUseOfcloneState({
            error: Error()
        ...
        bestPlanForUnit(state, actorId) {
          ...
          var stands = [(_crd && clone === void 0 ? (_reportPossibleCrUseOfclone({
            error: Error()
          ...
          for (var stand of stands) {
            var moveActions = (_crd && eq === void 0 ? (_reportPossibleCrUseOfeq({
              error: Error()
            ...
            var moveScore = this.scorePlan(state, moveActions, actorId);
            if (moveScore > best.score) best = {
              actions: ensureActed(moveActions, actorId),
              score: moveScore
            }; // 在该站位尝试每个技能

            var afterMove = moveActions.length ? this.applyAll(state, moveActions) : state;
            var movedActor = (_crd && unitById === void 0 ? (_reportPossibleCrUseOfunitById({
              error: Error()
            }), unitById) : unitById)(afterMove, actorId);
            if (!movedActor) continue;

            for (var skillId of movedActor.skills) {
              var skill = this.registry.skill(skillId);

              for (var target of this.skillTargets(afterMove, movedActor, skillId)) {
                ...
                var seq = [...moveActions, skillAction];
                var score = this.scorePlan(state, seq, actorId);
        ...
        skillTargets(state, actor, skillId) {
          ...
          if (skill.targetType === "direction") {
            for (var dir of ["up", "down", "left", "right"]) targets.push({
              direction: dir
            });
          ...
          if (skill.targetType === "unit") {
            for (var p of players) targets.push({
              unitId: p.instanceId,
          ...
          var castable = (_crd && getCastableCells === void 0 ? (_reportPossibleCrUseOfgetCastableCells({
            error: Error()
        ...
        applyAll(state, actions) {
          ...
          for (var a of actions) {
            var res = this.simulator.simulate(s, a);
        ...
        scorePlan(state, actions, actorId) {
          var result = this.applyAll(state, actions);
          var score = (_crd && evaluateForEnemy === void 0 ? (_reportPossibleCrUseOfevaluateForEnemy({
            error: Error()
        ...
        approachBonus(state, actor) {
          ...
          var d = Math.min(...players.map(p => (_crd && manhattan === void 0 ? (_reportPossibleCrUseOfmanhattan({
            error: Error()
