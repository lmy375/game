    ...
    execute: function () {
      ...
      _export("BoardView", BoardView = class BoardView {
        ...
        makeTile(p, terrain, coord) {
          ...
          var shape = (_SHAPE$terrain = SHAPE[terrain]) != null ? _SHAPE$terrain : SHAPE.ground;
          var size = coord.cell * 0.94;
          var mesh = (_crd && boxMesh === void 0 ? (_reportPossibleCrUseOfboxMesh({
            error: Error()
          }), boxMesh) : boxMesh)(size, shape.thick, size);
          var mat = (_crd && litMat === void 0 ? (_reportPossibleCrUseOflitMat({
            error: Error()
          ...
          var w = coord.posToWorld(p, shape.top - shape.thick / 2);
          n.setPosition(w);
          this.tiles.set(key(p), n); // 网格描边:细一圈深色边框立方体,凸显格子分界

          var edge = (_crd && meshNode === void 0 ? (_reportPossibleCrUseOfmeshNode({
            error: Error()
