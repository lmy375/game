System.register(["__unresolved_0", "cc", "__unresolved_1", "__unresolved_2"], function (_export, _context) {
  ...
  function computeMoveRange(state, instanceId) {
    var unit = state.units.find(u => u.instanceId === instanceId);
    if (!unit) return [];
    var start = unit.pos;
    var maxCost = unit.stats.moveRange;
    var dist = new Map();
    ...
    while (queue.length > 0) {
      var cur = queue.shift();
      var curCost = dist.get((_crd && key === void 0 ? (_reportPossibleCrUseOfkey({
        error: Error()
      }), key) : key)(cur));
  ...
  function findPath(state, instanceId, goal) {
    ...
    while (open.length > 0) {
      ...
      var curG = g.get((_crd && key === void 0 ? (_reportPossibleCrUseOfkey({
        error: Error()
      }), key) : key)(cur));
      ...
      var _loop = function _loop() {
        ...
        if (!state.board.isWalkable(next)) return 0; // continue

        var occ = (_crd && unitAt === void 0 ? (_reportPossibleCrUseOfunitAt({
          error: Error()
        }), unitAt) : unitAt)(state, next);
        if (occ && occ.instanceId !== instanceId) return 0; // continue

        var tentative = curG + 1;
