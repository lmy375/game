System.register(["__unresolved_0", "cc", "__unresolved_1", "__unresolved_2", "__unresolved_3", "__unresolved_4", "__unresolved_5", "__unresolved_6"], function (_export, _context) {
  ...
  function getCastableCells(state, caster, skill) {
    ...
    var within = (min, max) => {
      for (var x = 0; x < state.board.width; x++) {
        for (var y = 0; y < state.board.height; y++) {
          ...
          var d = (_crd && manhattan === void 0 ? (_reportPossibleCrUseOfmanhattan({
            error: Error()
    ...
    switch (range.type) {
      ...
      case "melee":
        for (var dir of _crd && ALL_DIRECTIONS === void 0 ? (_reportPossibleCrUseOfALL_DIRECTIONS({
          error: Error()
        }), ALL_DIRECTIONS) : ALL_DIRECTIONS) {
          var p = (_crd && add === void 0 ? (_reportPossibleCrUseOfadd({
            error: Error()
      ...
      case "direction":
        ...
        for (var _dir of _crd && ALL_DIRECTIONS === void 0 ? (_reportPossibleCrUseOfALL_DIRECTIONS({
          error: Error()
        }), ALL_DIRECTIONS) : ALL_DIRECTIONS) {
          var _p = (_crd && add === void 0 ? (_reportPossibleCrUseOfadd({
            error: Error()
          }), add) : add)(caster.pos, (_crd && DIRECTION_VECTOR === void 0 ? (_reportPossibleCrUseOfDIRECTION_VECTOR({
            error: Error()
          }), DIRECTION_VECTOR) : DIRECTION_VECTOR)[_dir]);

          if (state.board.inBounds(_p)) cells.push(_p);
        }
  ...
  function resolveTargeting(caster, skill, pattern, target) {
    ...
    var origin = pattern.anchor === "caster_direction" ? (_crd && clone === void 0 ? (_reportPossibleCrUseOfclone({
      error: Error()
  ...
  function resolveHitCells(caster, skill, registry, target) {
    var pattern = registry.pattern(skill.patternId);
    var targeting = resolveTargeting(caster, skill, pattern, target);
    var cells = (_crd && resolvePattern === void 0 ? (_reportPossibleCrUseOfresolvePattern({
      error: Error()
  ...
  function canCast(state, caster, skill, target) {
    ...
    if (skill.targetType === "unit") {
      var tu = target.unitId ? state.units.find(u => u.instanceId === target.unitId) : undefined;
    ...
    var candidates = getCastableCells(state, caster, skill);
  ...
  function resolveSkillEffects(state, caster, skill, registry, target, events) {
    ...
    var filter = (_skill$targetFilter = skill.targetFilter) != null ? _skill$targetFilter : "enemy"; // unit 目标转 cell

    var effectiveTarget = target;

    if (skill.targetType === "unit" && target.unitId) {
      var tu = state.units.find(u => u.instanceId === target.unitId);
    ...
    for (var cell of cells) {
      ...
      var u = (_crd && unitAt === void 0 ? (_reportPossibleCrUseOfunitAt({
        error: Error()
      ...
      var keyOps = (_skill$cellEffects$ce = skill.cellEffects[cell.effectKey]) != null ? _skill$cellEffects$ce : [];
      var allOps = cell.effectKey === "all" ? [] : (_skill$cellEffects$al = skill.cellEffects["all"]) != null ? _skill$cellEffects$al : [];
      var ops = [...keyOps, ...allOps];
      var existing = hitMap.get(u.instanceId);
    ...
    var displaced = new Set();
    var alreadyDead = new Set(state.units.filter(u => u.hp <= 0).map(u => u.instanceId)); // 2) 位移阶段
    // 2a) 群体整队 arrange_line（一次处理所有带该 op 的单位）

    var lineUnits = hits.filter(h => h.ops.some(o => o.type === "arrange_line"));

    if (lineUnits.length > 0) {
      var op = lineUnits[0].ops.find(o => o.type === "arrange_line");
    ...
    var gatherUnits = hits.filter(h => h.ops.some(o => o.type === "pull_to_center")).sort((a, b) => (_crd && manhattan === void 0 ? (_reportPossibleCrUseOfmanhattan({
      error: Error()
    ...
    for (var h of gatherUnits) {
      var _op = h.ops.find(o => o.type === "pull_to_center");

      var r = (_crd && applyPullToCenter === void 0 ? (_reportPossibleCrUseOfapplyPullToCenter({
        error: Error()
      }), applyPullToCenter) : applyPullToCenter)(state, h.unit, targeting.targetCell, _op.maxDistance, events);
      if (r.moved > 0) displaced.add(h.unit.instanceId);
    } // 2c) 逐个位移：push / pull / knockback / swap（按到施法者距离排序保证可预测）


    var ordered = [...hits].sort((a, b) => (_crd && manhattan === void 0 ? (_reportPossibleCrUseOfmanhattan({
      error: Error()
    ...
    for (var _h of ordered) {
      for (var _op2 of _h.ops) {
        if (_op2.type === "push") {
          var _r = (_crd && applyPush === void 0 ? (_reportPossibleCrUseOfapplyPush({
            error: Error()
          }), applyPush) : applyPush)(state, _h.unit, targeting.dir, _op2.distance, events);

          if (_r.moved > 0) displaced.add(_h.unit.instanceId);
        } else if (_op2.type === "pull") {
          var _r2 = (_crd && applyPull === void 0 ? (_reportPossibleCrUseOfapplyPull({
            error: Error()
          }), applyPull) : applyPull)(state, _h.unit, caster.pos, _op2.distance, events);

          if (_r2.moved > 0) displaced.add(_h.unit.instanceId);
        } else if (_op2.type === "knockback") {
          var _r3 = (_crd && applyKnockback === void 0 ? (_reportPossibleCrUseOfapplyKnockback({
            error: Error()
          }), applyKnockback) : applyKnockback)(state, _h.unit, targeting.dir, _op2.distance, _op2.collisionDamage, events);

          if (_r3.moved > 0) displaced.add(_h.unit.instanceId);
        } else if (_op2.type === "swap") {
          if ((_crd && applySwap === void 0 ? (_reportPossibleCrUseOfapplySwap({
            error: Error()
          }), applySwap) : applySwap)(state, caster, _h.unit, events)) {
            displaced.add(_h.unit.instanceId);
    ...
    var _loop = function _loop(id) {
      var u = state.units.find(x => x.instanceId === id);
      if (u) (_crd && triggerTerrain === void 0 ? (_reportPossibleCrUseOftriggerTerrain({
        error: Error()
      }), triggerTerrain) : triggerTerrain)(state, u, events);
    };

    for (var id of displaced) {
      _loop(id);
    } // 4) 伤害阶段


    for (var _h2 of hits) {
      for (var _op3 of _h2.ops) {
        if (_op3.type === "damage") {
          (_crd && dealDamage === void 0 ? (_reportPossibleCrUseOfdealDamage({
            error: Error()
          }), dealDamage) : dealDamage)(state, _h2.unit, (_crd && computeDamage === void 0 ? (_reportPossibleCrUseOfcomputeDamage({
            error: Error()
          }), computeDamage) : computeDamage)(caster, _h2.unit, _op3.element, _op3.multiplier), "skill:" + skill.id, events);
        } else if (_op3.type === "heal") {
          (_crd && heal === void 0 ? (_reportPossibleCrUseOfheal({
            error: Error()
          }), heal) : heal)(state, _h2.unit, Math.round(caster.stats.magic * _op3.multiplier), events);
        }
      }
    } // 5) 状态阶段
    ...
    for (var _h3 of hits) {
      for (var _op4 of _h3.ops) {
        if (_op4.type === "apply_status") {
          (_crd && applyStatus === void 0 ? (_reportPossibleCrUseOfapplyStatus({
            error: Error()
          }), applyStatus) : applyStatus)(_h3.unit, _op4.status, _op4.duration, _op4.magnitude, events);
        }
      }
    } // 6) 死亡判定
