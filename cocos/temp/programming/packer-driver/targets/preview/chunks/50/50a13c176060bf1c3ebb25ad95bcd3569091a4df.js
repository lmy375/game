  ...
  return {
    ...
    execute: function () {
      ...
      _export("SkillEffects", SkillEffects = class SkillEffects {
        ...
        burst(atWorld, color, count, radius) {
          var _this = this;
          ...
          var at = this.toUI(atWorld);

          var _loop = function _loop() {
            var dot = (_crd && uiGraphics === void 0 ? (_reportPossibleCrUseOfuiGraphics({
              error: Error()
            }), uiGraphics) : uiGraphics)(_this.layer);
            ...
            var delta = new Vec3(Math.cos(ang) * r, Math.sin(ang) * r, 0);
        ...
        ring(atWorld, color, maxScale, life) {
          if (maxScale === void 0) {
            maxScale = 3;
          }
          ...
          var at = this.toUI(atWorld);
          var g = (_crd && uiGraphics === void 0 ? (_reportPossibleCrUseOfuiGraphics({
            error: Error()
        ...
        projectile(fromWorld, toWorld, color, onArrive) {
          var from = this.toUI(fromWorld);
          var to = this.toUI(toWorld);
          var dot = (_crd && uiGraphics === void 0 ? (_reportPossibleCrUseOfuiGraphics({
            error: Error()
          }), uiGraphics) : uiGraphics)(this.layer);
          dot.fillColor = color;
          ...
          var dist = Vec3.distance(from, to);
          var life = Math.max(0.12, Math.min(0.4, dist / 1400));
          tween(dot.node).to(life, {
            position: to
          }, {
            easing: "quadIn"
        ...
        cast(skillId, casterWorld, targetWorld) {
          var tgt = targetWorld != null ? targetWorld : casterWorld;
