System.register(["__unresolved_0", "cc", "__unresolved_1", "__unresolved_2", "__unresolved_3"], function (_export, _context) {
  ...
  function tickUntilReady(state) {
    ...
    const alive = (_crd && livingUnits === void 0 ? (_reportPossibleCrUseOflivingUnits({
      error: Error()
  ...
  function startUnitTurn(state, unit, events) {
    ...
    const alreadyDead = new Set(state.units.filter(u => u.hp <= 0).map(u => u.instanceId));
  ...
  function predictTurnOrder(state, n) {
    ...
    const sim = (_crd && livingUnits === void 0 ? (_reportPossibleCrUseOflivingUnits({
      error: Error()
    ...
    while (order.length < n && guard++ < 100000) {
      ...
      const ready = sim.filter(u => u.ct >= CT_THRESHOLD).sort((a, b) => b.ct !== a.ct ? b.ct - a.ct : b.speed - a.speed);
      const pick = ready[0];
      order.push(pick.id);
